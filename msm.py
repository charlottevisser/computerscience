...
from binary_vectors import get_model_words_titles, get_unique_model_words_features
    ...
    for wordA in modelWordsA: 
        for wordB in modelWordsB: 
            ...
            if numericA != numericB and textdistance.levenshtein.normalized_similarity(non_numericA, non_numericB) > 0.5:
                # print(f"nonnumericA: {non_numericA}, numericA: {numericA}, nonnumericB: {non_numericB}, numericB: {numericB}")
                return -1
...
def MSM(product_list, pairs, gamma, alpha, beta, mu, delta, epsilon_TMWM, epsilon_clustering):
    ...
    for pi_x, pj_y in pairs: 
        ...
        # if (pi_brand != pj_brand and pi_brand is not None and pj_brand is not None):
            continue
